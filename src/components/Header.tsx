...
export default function Header() {
  ...
  const navLinks = [
    { path: "/", name: "Home" },
    { path: "/downloadApp", name: "Download App" },
    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-[#1B262C]/70 backdrop-blur-md text-white shadow-md">
      <div className="container mx-auto flex justify-between items-center py-4 px-6 md:px-12">
        ...
        <nav className="hidden lg:flex gap-10 text-lg">
          {navLinks.map((link) => (
            <Link key={link.path} href={link.path} className={`hidden lg:flex hover:text-[#00ADB5] transition-colors ${pathname === link.path ? "text-[#00ADB5]" : ""}`}>
              {link.name}
            </Link>
          ))}
        </nav>
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden text-3xl focus:outline-none cursor-pointer" aria-label="Toggle menu">
      ...
      {menuOpen && (
        <div className="lg:hidden bg-[#1B262C]/90 backdrop-blur-xl flex flex-col items-center pb-4 text-lg animate-slideDown">
          {navLinks.map((link) => (
            <Link key={link.path} href={link.path} className={`hover:text-[#00ADB5] w-full text-center py-2 hover:bg-[#172b36] ${pathname === link.path ? "text-[#00ADB5] bg-[#172b36]" : ""}`}>
              {link.name}
            </Link>
          ))}
          <Link href="/register" className="bg-[#00ADB5] hover:bg-[#008C96] px-4 py-2.5 rounded-xl text-sm font-medium mt-2">
            Sign Up
          </Link>
        </div>
      )}
